...
$w2 = Send $sal "GET" "/workflow/tasks" $null; Check "sales workflow tasks (menu perm)" ($w2.code -eq 0) "tasks=$($w2.data.tasks.Count)"
...
$ctId = "$($ct1.data)"
$ct2 = Send $sal "POST" "/contracts/$ctId/sign" @{}; Check "sales sign contract (has contract:manage)" ($ct2.code -eq 0) "code=$($ct2.code)"
$ct3 = Send $sal "POST" "/contracts/$ctId/sign" @{}; Check "double-sign rejected" ($ct3.code -ne 0) "code=$($ct3.code)"
...
Check "stat summary (sales has 906)" ((Send $sal "GET" "/stat/summary?dim=MONTH" $null).code -eq 0) "code=0"
$st2 = Send $sal "POST" "/stat/rebuild" @{}; Check "sales stat rebuild (has 906) -- OVER-GRANT?" ($st2.code -eq 0) "code=$($st2.code) (sales can trigger maintenance rebuild)"
